...
#include "../include/coup_project.hxx"  // coup_project, coup_project::create_project
...
class clean_command {
 public:
  bool execute() const noexcept { return true; }
};
...
inline bool execute_cmd(const command_type& cmd) {
  return std::visit([](const auto& c) { return c.execute(); }, cmd);
}
