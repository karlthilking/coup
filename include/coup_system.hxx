...
inline std::string make_compile_and_link_command(const std::vector<fs::path>& src_files);
inline std::string make_run_command(const fs::path& exec_file);
inline std::string make_mm_command(const fs::path& src_file, const fs::path& dep_file);
inline std::string make_system_command(const std::string& command, const fs::path& file);
