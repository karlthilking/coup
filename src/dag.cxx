...
node::node(const node& other)
    : f_type(other.f_type), f_path(other.f_path), children(other.children) {}
...
void node::insert(node* v) noexcept { children.push_back(v); }
...
// -------------------- DAG ----------------------
dag::dag(const std::vector<node*>& S) noexcept : nodes(S) {}
...
std::vector<node*> dag::merge_node_vecs(std::vector<node*>&& src_nodes,
                                        std::vector<node*>&& header_nodes,
                                        std::vector<node*>&& obj_nodes) {
  src_nodes.reserve(src_nodes.size() + header_nodes.size() + obj_nodes.size());

  src_nodes.insert(end(src_nodes), begin(header_nodes), end(header_nodes));

  src_nodes.insert(end(src_nodes), begin(obj_nodes), end(obj_nodes));

  return src_nodes;
}

std::vector<node*> dag::create_node_vec(std::vector<fs::path>&& src_files,
  ...
  std::vector<node*> nodes = merge_node_vecs(
      std::move(src_nodes), std::move(header_nodes), std::move(obj_nodes));
  return nodes;
}
...
dag dag::create_dag() {
  ...
  if (!r_opt.has_value()) {
    throw std::runtime_error(
        "[ERROR] No root directory found. Define a src or include directory to "
  ...
  std::vector<node*> dag_nodes = create_node_vec(
      std::move(src_files), std::move(header_files), std::move(obj_files));
