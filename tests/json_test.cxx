...
TEST_F(test_json, test_constructor)
{
    ...
    EXPECT_THROW(bad = coup_json(fs::path(BAD_CONFIG)),
                 std::runtime_error);
}
...
TEST_F(test_json, get_source_directories)
{
    ...
    sources = good.get_source_directories();
...
TEST_F(test_json, get_compile_flags)
{
    good = coup_json(fs::path(GOOD_CONFIG));
    std::vector<std::string>
    compile_flags = good.get_compile_flags();
    
    auto find_wall = std::find(begin(compile_flags),
                               end(compile_flags),
                               "-Wall");
    auto find_wextra = std::find(begin(compile_flags),
                                 end(compile_flags),
                                 "-Wextra");
    auto find_wpedantic = std::find(begin(compile_flags),
                                    end(compile_flags),
                                    "-Wpedantic");
